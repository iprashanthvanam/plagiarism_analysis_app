...
const DelegateCard = ({ name, role, img }: { name: string, role: string, img: string }) => (
  <div className="bg-white rounded-lg overflow-hidden shadow-lg hover:shadow-2xl hover:scale-105 transition-all duration-300 flex flex-col items-center p-4 border border-blue-900 h-full">
    ...
    <div className="text-center w-full flex flex-col justify-between flex-1">
      <div className="bg-black text-white py-1 px-2 rounded-full text-xs font-bold uppercase mb-2 shadow-md inline-block mx-auto">
        {name}
      </div>
...
export function LoginPage() {
  ...
  return (
    // Main Container with padding-bottom to prevent footer overlap on mobile
    <div className="min-h-screen bg-gray-50 flex flex-col font-sans overflow-x-hidden pb-28 md:pb-24">
      
      {/* --- HERO BANNER --- */}
      {/* Aligned with max-w grid below */}
      <div className="w-full max-w-[1440px] mx-auto px-4 md:px-6 pt-4 md:pt-6">
        <div className="relative w-full h-auto overflow-hidden rounded-lg shadow-md">
          {/* h-auto + object-contain prevents the zoomed-in cropping issue */}
          <img 
            src="/assets/logo.jpg" 
            alt="College Campus" 
            className="w-full h-auto object-contain"
      ...
      <div className="w-full max-w-[1440px] mx-auto p-4 md:p-6">
        {/* FIX: Using 'md:' breakpoint triggers the desktop layout on "Desktop Mode" in mobile */}
        <div className="w-full grid grid-cols-1 md:grid-cols-4 gap-6">
          
          {/* LEFT: ORIENTATION SLIDER */}
          {/* FIX: md:col-span-3 ensures it takes 75% width on tablet/desktop mode */}
          <div className="md:col-span-3 h-[250px] sm:h-[400px] lg:h-[650px]">
            <OrientationSlider />
          </div>

          {/* RIGHT: LOGIN & LINKS */}
          {/* FIX: md:col-span-1 ensures it sits to the right on tablet/desktop mode */}
      ...
      <div className="w-full max-w-[1440px] mx-auto pb-12 px-4 md:px-6">
        {/* FIX: md:grid-cols-4 ensures side-by-side on desktop mode in mobile */}
        <div className="w-full grid grid-cols-1 md:grid-cols-4 gap-6">
          ...
          <div className="md:col-span-3 flex flex-col gap-8 md:gap-12">
             ...
             {/* 1. ABOUT / VISION / MISSION */}
             <div>
                {/* FIX: md:grid-cols-4 puts them in one row on desktop mode */}
             ...
             <div>
                ...
                {/* FIX: md:grid-cols-5 ensures single row on desktop mode in mobile */}
          ...
          {/* --- RIGHT COLUMN: CONTACT INFO (Sidebar) --- */}
          {/* FIX: md:col-span-1 puts sidebar on the right on desktop mode */}
